...
AuctionatorScrollListMixin = CreateFromMixins(ScrollListMixin, AuctionatorAdvancedSearchProviderMixin)
...
function AuctionatorScrollListMixin:OnLoad()
  ...
  self.searchProvider:InitSearch(
    ...
    function(current, total, results)
      ...
      self.InsetFrame.ResultsText:SetText(Auctionator.Locales.Apply("LIST_SEARCH_STATUS", current, total, self.currentList.name))
    end
  )
end
...
function AuctionatorScrollListMixin:StartSearch(searchTerms)
  self.InsetFrame.ResultsText:SetText(Auctionator.Locales.Apply("LIST_SEARCH_START", self.currentList.name))
  self.InsetFrame.ResultsText:Show()

  self.InsetFrame.LoadingSpinner.SpinnerAnim:Play()
  self.InsetFrame.LoadingSpinner:Show()
...
function AuctionatorScrollListMixin:HideSpinner()
  self.InsetFrame.LoadingSpinner:Hide()
  self.InsetFrame.ResultsText:Hide()
end
