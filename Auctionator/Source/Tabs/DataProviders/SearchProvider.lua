...
function AuctionatorSearchDataProviderMixin:OnShow()
  FrameUtil.RegisterFrameForCustomEvents(self, SEARCH_EVENTS)
...
function AuctionatorSearchDataProviderMixin:OnHide()
  FrameUtil.UnregisterFrameForCustomEvents(self, SEARCH_EVENTS)
...
function AuctionatorSearchDataProviderMixin:ReceiveEvent(eventName, itemKey)
  ...
  elseif eventName == Auctionator.Cancelling.Events.RequestCancel then
    self:RegisterCustomEvent("AUCTION_CANCELED")
  end
end
...
function AuctionatorSearchDataProviderMixin:ProcessCommodityResults(itemID)
  ...
  for index = C_AuctionHouse.GetNumCommoditySearchResults(itemID), 1, -1 do
    ...
    local entry = {
      ...
      level = 0,
      levelPretty = "0",
      timeLeftPretty = Auctionator.Utilities.FormatTimeLeft(resultInfo.timeLeftSeconds or 0),
